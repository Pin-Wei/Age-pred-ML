...
class Config:
    ...
    def setup_paths(self, args):
        ...
        self.output_dir = os.path.join(self.root_dir, "outputs", f"{datetime.today().strftime('%Y-%m-%d')}_simple")
    ...
    def setup_vars(self, args):
        ...
        self.student_model_name = "XGBoost" # "ElasticNet"
        self.opt_trial_num = 200
...
def build_model(model_name, params, config):
    if model_name == "XGBoost":
        return XGBRegressor( # see: https://xgboost.readthedocs.io/en/release_3.0.0/parameter.html
...
def optimize_hyperparameters(X, y, grp, model_name, config, logger):
    def _objective(trial, X, y, grp, model_name, config, logger):
        ...
        model = build_model(model_name, params, config)
        skf = StratifiedKFold(
            n_splits=config.k_folds, shuffle=True, random_state=config.seed
        )
        mae_list = []
        for n, (tr_idx, va_idx) in enumerate(skf.split(X.index, grp)):
            ...
            logger.info(f"Trial {trial.number}, Fold {n+1}: MAE = {mae:.3f}")
            mae_list.append(mae)
            
            trial.report(np.mean(mae_list), n) # report intermediate objective value
    ...
    logger.info(f"Optimization is done. The best mean MAE is {study.best_value:.3f} in trial {study.best_trial.number}.")

    return best_params

def trainval_student(DF_trainval, feature_available, params, res_records, mae_records, config, logger):
    for n in range(config.k_folds):
        logger.info(f"Training student model (fold {n+1}/{config.k_folds}) ...")
        student_model = build_model(config.student_model_name, params, config)
        ...
        res_records.append(pd.DataFrame({
            "Model": "Student",
            "Stage": "Val", 
            "Fold": n+1, 
            "ID": DF_val["ID"].tolist(),
            "Real_Age": DF_val["Age"].tolist(), 
            "Pred_Age": y_va_pred.tolist()
        }, index=DF_val.index))
        mae = mean_absolute_error(DF_val["Age"], y_va_pred)
        logger.info(f"Fold {n+1}: MAE = {mae:.3f}")
        mae_records.append(pd.DataFrame({
            "Model": "Student",
            "Stage": "Val", 
            "Fold": n+1, 
            "MAE": mae, 
    ...
    df3 = pd.concat(mae_records, ignore_index=True).query("Model == 'Student' & Stage == 'Val'")
    best_fold_student = df3.loc[df3["MAE"].idxmin(), "Fold"]
    ...
    df4 = pd.concat(res_records, ignore_index=True).query(f"Model == 'Student' & Stage == 'Val' & Fold == {best_fold_student}")
    ...
    return student_model, slope_st, const_st, mae_records, res_records
...
def main():
    ...
    if config.student_model_name == "ElasticNet":
    ...
    student_model, slope_st, const_st, mae_records, res_records = trainval_student(
        DF_trainval, feature_available, hyperparams_student, res_records, mae_records, config, logger
    )

    X_test_platform = DF_test.loc[:, feature_available]
    if config.student_model_name == "ElasticNet":
        X_test_platform = imputer.transform(X_test_platform)
    ...
    mae = mean_absolute_error(DF_test["Age"], y_test_pred_student)
    mae_records.append(pd.DataFrame({
        "Model": "Student",
        "Stage": "Test",
        "Fold": "", 
    ...
    logger.info(f"MAE of student model on test set = {mae:.3f}")

    ## Finally -----------------------------------------------------------------------
    
    pd.concat(res_records, ignore_index=True).to_csv(config.predictions_path, index=False)
    pd.concat(mae_records, ignore_index=True).to_csv(config.performance_path, index=False)
